...
async function performLogin(rawLogin, rawPass, isAlreadyHashed, uid = null) {
  ...
  try {
    ...
    if (res.status === 404 && uid) {
      ...
      // Немає існуючого акаунту → одразу створюємо
      if (!existingLogin) {
...
function createRecordCard(rec){
  ...
  card.innerHTML=`${!rec.paid?'<div class="absolute top-0 right-0 w-20 h-20 bg-gradient-to-bl from-orange-400/15 to-transparent rounded-bl-[4rem]"></div>':''}<div class="flex justify-between items-center relative z-10" data-toggle-details="${recId}"><div><h4 class="font-bold text-xl capitalize text-slate-900 dark:text-white mb-1.5">${escapeHtml(dStr)}</h4><div class="flex items-center flex-wrap gap-1"><span class="text-[9px] font-black uppercase tracking-widest px-2.5 py-1 rounded-lg ${rec.paid?'bg-brand-light text-brand':'bg-orange-100 text-orange-700 dark:bg-orange-500/20 dark:text-orange-400'}">${rec.paid?'Оплачено':'Борг'}</span>${partialBadge}${yoy}</div></div><div class="flex items-center gap-3"><span class="font-black text-2xl text-slate-900 dark:text-white">${fmt.format(rec.total)} ₴</span><div class="w-8 h-8 flex items-center justify-center bg-slate-50 dark:bg-white/5 rounded-full text-slate-400"><i class="chevron-icon fa-solid fa-chevron-down transition-transform duration-300"></i></div></div></div><div class="details-panel hidden"><div class="border-t border-slate-100 dark:border-white/5 pt-5 mt-5">${rec.total>0?`<div class="flex items-center gap-4 bg-slate-50 dark:bg-black/50 p-4 rounded-2xl border border-slate-100 dark:border-white/5 mb-5"><div class="w-14 h-14 rounded-full shrink-0 shadow-sm border border-slate-200 dark:border-white/10" style="background:${conic}"></div><div class="flex flex-col gap-1 text-[10px] font-bold text-slate-500 w-full">${pW>0?`<div class="flex justify-between"><span>💧 Вода</span><span>${Math.round(pW)}%</span></div>`:''}${pHW>0?`<div class="flex justify-between"><span>🌡️ Гар.</span><span>${Math.round(pHW)}%</span></div>`:''}${pE>0?`<div class="flex justify-between"><span>⚡ Світло</span><span>${Math.round(pE)}%</span></div>`:''}${pG>0?`<div class="flex justify-between"><span>🔥 Газ</span><span>${Math.round(pG)}%</span></div>`:''}${(100-pW-pHW-pE-pG)>1?`<div class="flex justify-between"><span>📦 Інше</span><span>${Math.round(100-pW-pHW-pE-pG)}%</span></div>`:''}</div></div>`:''}
  <div class="space-y-3">${rec.waterCost>0?`<div class="flex justify-between"><span class="font-bold">💧 Вода</span><span class="font-black">${fmt.format(rec.waterCost)} ₴</span></div><div class="flex justify-between text-[11px] font-bold text-slate-500 bg-slate-50 dark:bg-black/50 px-3 py-2 rounded-xl"><span>${rec.wPrev}→${rec.wCur}</span><span class="text-blue-500">+${rec.wCur-rec.wPrev} м³</span></div>`:''}${rec.hotWaterCost>0?`<div class="flex justify-between"><span class="font-bold">🌡️ Гар.</span><span class="font-black">${fmt.format(rec.hotWaterCost)} ₴</span></div><div class="flex justify-between text-[11px] font-bold text-slate-500 bg-slate-50 dark:bg-black/50 px-3 py-2 rounded-xl"><span>${rec.hwPrev}→${rec.hwCur}</span><span class="text-red-500">+${rec.hwCur-rec.hwPrev} м³</span></div>`:''}${rec.electroCost>0?`<div class="flex justify-between"><span class="font-bold">⚡ Світло</span><span class="font-black">${fmt.format(rec.electroCost)} ₴</span></div><div class="flex justify-between text-[11px] font-bold text-slate-500 bg-slate-50 dark:bg-black/50 px-3 py-2 rounded-xl"><span>Д:${rec.dPrev}→${rec.dCur}</span><span class="text-yellow-600">+${rec.dCur-rec.dPrev}</span></div>${(rec.nCur||rec.nPrev)?`<div class="flex justify-between text-[11px] font-bold text-slate-500 bg-slate-50 dark:bg-black/50 px-3 py-2 rounded-xl mt-1"><span>Н:${rec.nPrev}→${rec.nCur}</span><span class="text-indigo-500">+${rec.nCur-rec.nPrev}</span></div>`:''}`:''}${rec.gasCost>0?`<div class="flex justify-between"><span class="font-bold">🔥 Газ</span><span class="font-black">${fmt.format(rec.gasCost)} ₴</span></div><div class="flex justify-between text-[11px] font-bold text-slate-500 bg-slate-50 dark:bg-black/50 px-3 py-2 rounded-xl"><span>${rec.gPrev}→${rec.gCur}</span><span class="text-orange-500">+${rec.gCur-rec.gPrev} м³</span></div>`:''}${rec.customCost>0?`<div class="flex justify-between"><span class="font-bold">📦 Інше</span><span class="font-black">${fmt.format(rec.customCost)} ₴</span></div>${rec.customData?Object.values(rec.customData).filter(s=>s.val>0).map(s=>`<div class="flex justify-between text-[11px] font-bold text-slate-500 bg-slate-50 dark:bg-black/50 px-3 py-2 rounded-xl"><span>${escapeHtml(s.name)}</span><span class="text-purple-500">${fmt.format(s.val)} ₴</span></div>`).join(''):''}`:'' }${rec.note?`<div class="mt-3 p-3 bg-slate-50 dark:bg-black/50 rounded-xl text-xs text-slate-500 italic"><i class="fa-solid fa-sticky-note mr-1"></i>${escapeHtml(rec.note)}</div>`:''}</div></div>
  <div class="flex gap-2.5 mt-4 pt-3 border-t border-slate-100 dark:border-white/5"><button type="button" class="rec-pay flex-1 py-3.5 rounded-2xl font-bold text-xs border active:scale-[0.96] transition-all ${rec.paid?'bg-slate-50 dark:bg-[#2c2c2e] text-slate-500 border-slate-200 dark:border-white/10':'bg-gradient-to-r from-brand to-blue-600 text-white shadow-lg border-brand'}" data-rec-id="${recId}">${rec.paid?'↩ Скасувати':'✓ Оплачено'}</button><button type="button" class="rec-share w-12 bg-blue-50 dark:bg-blue-500/10 rounded-2xl text-blue-500 active:scale-[0.90] transition-transform" data-rec-id="${recId}"><i class="fa-solid fa-share-nodes"></i></button><button type="button" class="rec-edit w-12 bg-slate-50 dark:bg-white/5 rounded-2xl text-slate-400 active:scale-[0.90] transition-transform" data-rec-id="${recId}"><i class="fa-solid fa-pen"></i></button><button type="button" class="rec-del w-12 bg-red-50 dark:bg-red-500/10 rounded-2xl text-red-400 active:scale-[0.90] transition-transform" data-rec-id="${recId}"><i class="fa-solid fa-trash"></i></button></div></div>`;
...
function normalizeImportedAddress(addr,index){if(!isPlainObject(addr))return null;const normalizedRecords=Array.isArray(addr.records)?addr.records.map(normalizeImportedRecord).filter(Boolean):[];return{id:String(addr.id||`addr_${Date.now()}_${index}`),name:String(addr.name||`Об'єкт ${index+1}`).slice(0,80),tariffs:{...defaultTariffs,...(isPlainObject(addr.tariffs)?addr.tariffs:{})},prefs:{...defaultPrefs,...(isPlainObject(addr.prefs)?addr.prefs:{})},records:normalizedRecords,customServices:Array.isArray(addr.customServices)?addr.customServices.filter(isPlainObject).map((srv,srvIndex)=>({id:String(srv.id||`srv_${Date.now()}_${srvIndex}`),name:String(srv.name||'').slice(0,60),defaultSum:srv.defaultSum==null?'':String(srv.defaultSum).slice(0,20)})):[]};}
function normalizeImportData(data){if(!isPlainObject(data)||!Array.isArray(data.addresses))return null;const normalizedAddresses=data.addresses.map(normalizeImportedAddress).filter(Boolean);if(!normalizedAddresses.length)return null;const importedCurrentId=String(data.currentAddressId||'');return{addresses:normalizedAddresses,currentAddressId:normalizedAddresses.some(addr=>addr.id===importedCurrentId)?importedCurrentId:normalizedAddresses[0].id};}
...
function getSmartTips(){const tips=[];if(records.length>=3){const waterTrend=getConsumptionTrend('water');if(waterTrend&&waterTrend>20)tips.push({emoji:'💧',text:`Споживання води зросло на ${waterTrend}%. Перевірте крани.`});const electroTrend=getConsumptionTrend('electro');if(electroTrend&&electroTrend>20)tips.push({emoji:'⚡',text:`Електрика +${electroTrend}%. Перевірте прилади.`});if(electroTrend&&electroTrend<-10)tips.push({emoji:'🎉',text:`Електрика -${Math.abs(electroTrend)}%! Чудова економія!`});}
  ...
  if(prefs.showElectro&&prefs.electroTwoZone&&records.length>0){const last=[...records].sort((a,b)=>new Date(b.month)-new Date(a.month))[0];const nightUsage=Math.max(0,(last.nCur||0)-(last.nPrev||0)),dayUsage=Math.max(0,(last.dCur||0)-(last.dPrev||0)),total=nightUsage+dayUsage;if(total>0&&nightUsage/total<0.3)tips.push({emoji:'🌙',text:'Спробуйте більше електрики вночі — дешевше.'});}
...
function initAppUI(){
  ...
  // ── ВИПРАВЛЕННЯ МОБІЛЬНОГО AI ──
  // Ініціалізуємо AI після повного завантаження UI
  // Використовуємо requestAnimationFrame щоб DOM точно готовий
  requestAnimationFrame(() => {
    if (typeof initAI === 'function') {
      initAI();
    } else {
      // ai-chat.js ще не завантажився — чекаємо
