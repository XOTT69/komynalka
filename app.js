...
// =================== PERFORM LOGIN (БАГ GOOGLE ВИПРАВЛЕНО) ===================
async function performLogin(rawLogin, rawPass, isAlreadyHashed, uid = null) {
    ...
    try {
        ...
        if (res.status === 404 && uid) {
            ...
            // Є існуючий акаунт → пропонуємо прив'язати
...
// ── Link modal (прив'язка Google до існуючого логіну) ──
$('linkYesBtn')?.addEventListener('click', () => {
    const lgn = prompt("Введіть ваш існуючий логін:");
    if (!lgn) return;
    const pss = prompt("Введіть пароль:");
    if (pss) linkAccount(lgn, pss);
});
...
function checkNightOwl(recs) { if(!recs.length) return false; const last=[...recs].sort((a,b)=>new Date(b.month)-new Date(a.month))[0]; const n=Math.max(0,(last.nCur||0)-(last.nPrev||0)); const d=Math.max(0,(last.dCur||0)-(last.dPrev||0)); const t=n+d; return t>0&&(n/t)>=0.7; }
...
function showAchievementUnlock(ach) {
    ...
    $('achievementEmoji').textContent=ach.emoji; $('achievementTitle').textContent=ach.title; $('achievementDesc').textContent=ach.desc;
...
function showAchievementDetail(achId) {
    ...
    $('achDetailEmoji').textContent=ach.emoji; $('achDetailTitle').textContent=ach.title; $('achDetailDesc').textContent=ach.desc; $('achDetailHow').textContent=ACHIEVEMENT_HINTS[achId]||'—';
    ...
    $('achievementDetailModal').classList.remove('hidden'); haptic('light');
}
...
class DonutChart {
    ...
    render() {
        ...
        data.forEach(d=>{
            const sliceAngle=(d.value/total)*Math.PI*2*this.animProgress,endAngle=startAngle+sliceAngle;
...
function updateSmartForecast(){
    ...
    if(sameMonth.length>0){el.innerText=`~ ${fmt.format(sameMonth.reduce((s,r)=>s+r.total,0)/sameMonth.length)} ₴`;return;}
    const sorted=[...records].sort((a,b)=>new Date(b.month)-new Date(a.month));
    el.innerText=`~ ${fmt.format(sorted.slice(0,3).reduce((s,r)=>s+r.total,0)/Math.min(3,sorted.length))} ₴`;
}
...
function checkReminders(){
    ...
    const d=new Date().getDate(),wS=prefs.remWaterStart||1,wE=prefs.remWaterEnd||5,eS=prefs.remElectroStart||28,eE=prefs.remElectroEnd||3;
...
$('changePassBtn')?.addEventListener('click',async()=>{
    ...
    try{
        const oldHash=await getHash(oldPass),newHash=await getHash(newPass);
...
function initSwipe(card,recordId){
    ...
    card.addEventListener('touchend',()=>{isSwiping=false;card.classList.remove('swiping');card.style.transform='';const l=card.querySelector('.swipe-bg-left'),r=card.querySelector('.swipe-bg-right');if(l)l.style.opacity='0';if(r)r.style.opacity='0';if(currentX<-80){card.style.transform='translateX(-100%)';card.style.opacity='0';setTimeout(()=>deleteRecordById(recordId),300);}else if(currentX>80){card.style.transform='translateX(100%)';card.style.opacity='0';setTimeout(()=>togglePaidById(recordId),300);}currentX=0;},{passive:true});
}
...
function renderRecords(){
    ...
    if(unpaidCount>0&&currentFilter!=='paid'){
        ...
        batchBar.querySelector('.batch-pay-btn')?.addEventListener('click',()=>{if(confirm(`Позначити ${unpaidCount} записів як оплачені?`)){records.forEach(r=>{if(!r.paid)r.paid=true;});renderRecords();renderDashboard();syncToCloud();checkNewAchievements();showToast(`${unpaidCount} записів оплачено!`,'✅');}});
    }
    let lastYear=null;
    sorted.forEach(rec=>{
...
function renderHistoryChart(sortedRecords,retryCount=0){
    ...
    historyChart.setData(sortedRecords.slice(-10).map(r=>({value:r.total,label:new Date(r.month+'-01').toLocaleString('uk-UA',{month:'short'}).slice(0,3),color:r.paid?'#007aff':'#ff9500'})));
}
...
function renderServiceChart(retryCount=0){
    ...
    const type=$('serviceChartSelect')?.value||'water',unit=type==='electro'?'кВт':'м³';
    if(!serviceChart)serviceChart=new ChartEngine('serviceChartCanvas',{padding:24,barRadius:4,unit});else serviceChart.options.unit=unit;
    ...
    const color=getColor(),data=sorted.map(rec=>({value:getValue(rec),label:new Date(rec.month+'-01').toLocaleString('uk-UA',{month:'short'}).slice(0,3),color}));
    serviceChart.setData(data);
    const values=data.map(d=>d.value),avg=values.length?values.reduce((a,b)=>a+b,0)/values.length:0,last=values[values.length-1]||0,prevLast=values.length>1?values[values.length-2]:last,trendPct=prevLast>0?Math.round(((last-prevLast)/prevLast)*100):0;
...
function createRecordCard(rec){
    ...
    card.addEventListener('click',(e)=>{
        ...
        if(toggleTarget){
            const panel=card.querySelector('.details-panel'),chevron=card.querySelector('.chevron-icon');
            if(panel){panel.classList.toggle('hidden');if(chevron)chevron.style.transform=panel.classList.contains('hidden')?'rotate(0deg)':'rotate(180deg)';}
            return;
        }
...
$('exportJsonBtn')?.addEventListener('click',()=>{
    syncCurrentAddress();
    downloadBlob(JSON.stringify({version:APP_VERSION,exportDate:new Date().toISOString(),addresses,currentAddressId},null,2),'komunalka_backup.json','application/json;charset=utf-8;');
    showToast('Бекап','💾');
});
...
function normalizeNumber(v,fb=0){const n=Number(v);return Number.isFinite(n)?n:fb;}
function normalizeImportedRecord(rec){
    if(!isPlainObject(rec)||!/^\d{4}-\d{2}$/.test(String(rec.month||'')))return null;
    return{...rec,id:rec.id||Date.now()+Math.random(),month:String(rec.month),total:normalizeNumber(rec.total),waterCost:normalizeNumber(rec.waterCost),hotWaterCost:normalizeNumber(rec.hotWaterCost),electroCost:normalizeNumber(rec.electroCost),gasCost:normalizeNumber(rec.gasCost),customCost:normalizeNumber(rec.customCost),paid:Boolean(rec.paid),note:String(rec.note||'')};
}
function normalizeImportedAddress(addr,index){
    if(!isPlainObject(addr))return null;
    return{id:String(addr.id||`addr_${Date.now()}_${index}`),name:String(addr.name||`Об'єкт ${index+1}`).slice(0,80),tariffs:{...defaultTariffs,...(isPlainObject(addr.tariffs)?addr.tariffs:{})},prefs:{...defaultPrefs,...(isPlainObject(addr.prefs)?addr.prefs:{})},records:Array.isArray(addr.records)?addr.records.map(normalizeImportedRecord).filter(Boolean):[],customServices:Array.isArray(addr.customServices)?addr.customServices.filter(isPlainObject).map((srv,i)=>({id:String(srv.id||`srv_${Date.now()}_${i}`),name:String(srv.name||'').slice(0,60),defaultSum:srv.defaultSum==null?'':String(srv.defaultSum).slice(0,20)})):[]};
}
function normalizeImportData(data){
    if(!isPlainObject(data)||!Array.isArray(data.addresses))return null;
    const addrs=data.addresses.map(normalizeImportedAddress).filter(Boolean);
    if(!addrs.length)return null;
    const cid=String(data.currentAddressId||'');
    return{addresses:addrs,currentAddressId:addrs.some(a=>a.id===cid)?cid:addrs[0].id};
}
$('importFileInput')?.addEventListener('change',(e)=>{
    const file=e.target.files[0];if(!file)return;
    const reader=new FileReader();
    reader.onload=ev=>{try{const n=normalizeImportData(JSON.parse(ev.target.result));if(!n){showToast('Невірний формат','❌');return;}if(confirm(`Імпорт ${n.addresses.length} об'єктів? Поточні дані буде замінено.`)){addresses=n.addresses;currentAddressId=n.currentAddressId;loadCurrentAddress();syncToCloud();showToast('Імпортовано!','✅');}}catch(err){showToast('Помилка','❌');}};
...
function getConsumptionTrend(type,months=6){
    ...
    const values=sorted.map(r=>{switch(type){case 'water':return Math.max(0,(r.wCur||0)-(r.wPrev||0));case 'electro':return Math.max(0,(r.dCur||0)-(r.dPrev||0))+Math.max(0,(r.nCur||0)-(r.nPrev||0));case 'gas':return Math.max(0,(r.gCur||0)-(r.gPrev||0));default:return r.total;}}).reverse();
    const first=values.slice(0,Math.ceil(values.length/2)),second=values.slice(Math.ceil(values.length/2));
    const avgF=first.reduce((a,b)=>a+b,0)/first.length,avgS=second.reduce((a,b)=>a+b,0)/second.length;
    if(avgF===0)return 0;
...
function getSmartTips(){
    const tips=[];
    if(records.length>=3){
        const wT=getConsumptionTrend('water');if(wT&&wT>20)tips.push({emoji:'💧',text:`Споживання води зросло на ${wT}%. Перевірте крани.`});
        const eT=getConsumptionTrend('electro');if(eT&&eT>20)tips.push({emoji:'⚡',text:`Електрика +${eT}%. Перевірте прилади.`});if(eT&&eT<-10)tips.push({emoji:'🎉',text:`Електрика -${Math.abs(eT)}%! Чудова економія!`});
    }
    const budget=parseFloat(localStorage.getItem('k_budget'))||0;
    if(budget&&records.length>0){const last=[...records].sort((a,b)=>new Date(b.month)-new Date(a.month))[0];if(last.total>budget*1.2)tips.push({emoji:'⚠️',text:`Перевищили бюджет на ${Math.round(((last.total-budget)/budget)*100)}%`});}
    const unpaid=records.filter(r=>!r.paid);if(unpaid.length>=3)tips.push({emoji:'💳',text:`${unpaid.length} неоплачених місяців. Оплатіть борг.`});
    if(prefs.showElectro&&prefs.electroTwoZone&&records.length>0){const last=[...records].sort((a,b)=>new Date(b.month)-new Date(a.month))[0];const n=Math.max(0,(last.nCur||0)-(last.nPrev||0)),d=Math.max(0,(last.dCur||0)-(last.dPrev||0)),tot=n+d;if(tot>0&&n/tot<0.3)tips.push({emoji:'🌙',text:'Спробуйте більше електрики вночі — дешевше.'});}
...
async function shareYearReport(){
    const year=new Date().getFullYear(),yr=records.filter(r=>r.month.startsWith(String(year)));if(!yr.length)return;
    const total=yr.reduce((s,r)=>s+r.total,0),avg=total/yr.length,streak=getStreak(records);
...
// =================== PWA ===================
let deferredPrompt,pendingServiceWorker=null,isRefreshingAfterUpdate=false;
...
async function initPush(){
    ...
    const btn=$('enablePushBtn'),st=$('pushStatus');
...
function scheduleLocalReminder(){
    ...
    const isW=wS<=wE?(d>=wS&&d<=wE):(d>=wS||d<=wE),isE=eS<=eE?(d>=eS&&d<=eE):(d>=eS||d<=eE);
    const monthKey=new Date().getFullYear()+'-'+new Date().getMonth();
    if((isW||isE)&&localStorage.getItem('lastSubmittedMonth')!==monthKey&&localStorage.getItem('lastPushShown')!==new Date().toDateString()){localStorage.setItem('lastPushShown',new Date().toDateString());new Notification('Комуналка 🏠',{body:'Час передати показники!',icon:'icon.png'});}
}
...
async function shareAddress(){
    ...
    try{
        ...
        if(navigator.share){try{await navigator.share({title:'Комуналка — гостьовий доступ',text:`Перегляд комунальних за "${addrName}"`,url:shareUrl});showToast('Надіслано!','✅');return;}catch(e){if(e.name==='AbortError')return;}}
        try{await navigator.clipboard.writeText(shareUrl);showToast('Посилання скопійовано!','📋');}catch(e){prompt('Скопіюйте посилання:',shareUrl);}
...
window.addEventListener('resize',()=>{
    ...
    resizeTimeout=setTimeout(()=>{
        [dashChart,historyChart,serviceChart,donutChart].forEach(chart=>{if(chart&&chart.canvas){chart.setupCanvas();if(chart.width)chart.render();}});
    },250);
});
...
async function registerServiceWorker(){
    ...
    try{
        ...
        registration.addEventListener('updatefound',()=>{
            const nextWorker=registration.installing;if(!nextWorker)return;
            nextWorker.addEventListener('statechange',()=>{if(nextWorker.state==='installed'&&navigator.serviceWorker.controller){pendingServiceWorker=nextWorker;showUpdateBanner();}});
        });
