...
function renderRecords() {
    ...
    if (unpaidCount > 0 && currentFilter !== 'paid') {
        ...
        batchBar.querySelector('.batch-pay-btn')?.addEventListener('click', () => {
            if (confirm(`Позначити ${unpaidCount} записів як оплачені?`)) { records.forEach(r => { if (!r.paid) r.paid = true; }); renderRecords(); renderDashboard(); syncToCloud(); checkNewAchievements(); showToast(`${unpaidCount} записів оплачено!`, '✅'); }
        });
    }
