// ============================================================
// КОМУНАЛКА PWA v3.3 — UI/UX Optimized
// ============================================================
...
const defaultTariffs = { water: 30.38, hotWater: 100.00, drainage: 28.89, electroBase: 4.32, electroWinter: 2.64, winterLimit: 2000, nightCoef: 0.5, gas: 7.96 };
const defaultPrefs = { showWater: true, showHotWater: false, showDrainage: true, showElectro: true, showGas: true, electroTwoZone: true, electroWinter: true, remindersEnabled: false, remWaterStart: 1, remWaterEnd: 5, remElectroStart: 28, remElectroEnd: 3 };
const defaultCustomServices = [
    { id: "s_rent", name: "Квартплата", defaultSum: "" },
    { id: "s_garbage", name: "Вивіз сміття", defaultSum: "" },
    { id: "s_heating", name: "Опалення", defaultSum: "" },
    { id: "s_elevator", name: "Ліфт", defaultSum: "" },
    { id: "s_internet", name: "Інтернет", defaultSum: "" },
    { id: "s_intercom", name: "Домофон", defaultSum: "" }
];

let addresses = [], currentAddressId = 'default', isGuest = false, tariffs = {}, prefs = {}, records = [], customServices = [];
let currentCalc = { waterCost: 0, hotWaterCost: 0, drainageCost: 0, electroCost: 0, gasCost: 0, customCost: 0, total: 0 };
const urlParams = new URLSearchParams(window.location.search), urlShareToken = urlParams.get('share');

// =================== CITY TARIFF TEMPLATES ===================
const CITY_TARIFFS = {
    'kyiv': { name: 'Київ', water: 30.38, hotWater: 100.98, drainage: 28.89, electroBase: 4.32, electroWinter: 2.64, gas: 7.96 },
    'kharkiv': { name: 'Харків', water: 27.50, hotWater: 89.64, drainage: 18.40, electroBase: 4.32, electroWinter: 2.64, gas: 7.96 },
    'odesa': { name: 'Одеса', water: 29.81, hotWater: 95.00, drainage: 22.75, electroBase: 4.32, electroWinter: 2.64, gas: 7.96 },
    'dnipro': { name: 'Дніпро', water: 28.08, hotWater: 87.36, drainage: 19.44, electroBase: 4.32, electroWinter: 2.64, gas: 7.96 },
    'lviv': { name: 'Львів', water: 31.20, hotWater: 78.50, drainage: 24.30, electroBase: 4.32, electroWinter: 2.64, gas: 7.96 },
    'zaporizhzhia': { name: 'Запоріжжя', water: 25.92, hotWater: 82.00, drainage: 17.28, electroBase: 4.32, electroWinter: 2.64, gas: 7.96 },
    'vinnytsia': { name: 'Вінниця', water: 26.40, hotWater: 75.00, drainage: 20.16, electroBase: 4.32, electroWinter: 2.64, gas: 7.96 },
    'poltava': { name: 'Полтава', water: 24.30, hotWater: 70.00, drainage: 16.80, electroBase: 4.32, electroWinter: 2.64, gas: 7.96 },
    'custom': { name: 'Інше (вручну)', water: 30.00, hotWater: 90.00, drainage: 25.00, electroBase: 4.32, electroWinter: 2.64, gas: 7.96 }
};

function applyCityTariffs(cityId) {
    const city = CITY_TARIFFS[cityId];
    if (!city) return;
    tariffs = { ...tariffs, ...city };
    delete tariffs.name;
    if ($('tWater')) $('tWater').value = tariffs.water;
    if ($('tHotWater')) $('tHotWater').value = tariffs.hotWater;
    if ($('tDrainage')) $('tDrainage').value = tariffs.drainage;
    ...
    showToast(`Тарифи: ${city.name}`, '🏙️');
}
window.applyCityTariffs = applyCityTariffs;
...
// =================== UTILS ===================
let toastTimeout;
function showToast(msg, icon = '✅') { const t = $('toast'); if (!t) return; $('toastMsg').innerText = msg; $('toastIcon').innerText = icon; t.classList.remove('-translate-y-24', 'opacity-0'); haptic(icon === '✅' ? 'success' : icon === '❌' || icon === '⚠️' ? 'error' : 'notification'); clearTimeout(toastTimeout); toastTimeout = setTimeout(() => t.classList.add('-translate-y-24', 'opacity-0'), 3500); }

function showSaveSummary(action, rec) {
    if (!rec) { showToast(`${action}!`, '✅'); return; }
    const parts = [];
    if (rec.waterCost > 0) parts.push(`💧${Math.round(rec.waterCost)}`);
    if (rec.hotWaterCost > 0) parts.push(`🌡${Math.round(rec.hotWaterCost)}`);
    if (rec.electroCost > 0) parts.push(`⚡${Math.round(rec.electroCost)}`);
    if (rec.gasCost > 0) parts.push(`🔥${Math.round(rec.gasCost)}`);
    if (rec.customCost > 0) parts.push(`📦${Math.round(rec.customCost)}`);
    const summary = parts.length ? `${parts.join('+')} = ${fmt.format(rec.total)}₴` : `${fmt.format(rec.total)}₴`;
    showToast(`${action}: ${summary}`, '✅');
}
...
async function getDeviceFingerprint() {
    const components = [navigator.userAgent, navigator.language, screen.width + 'x' + screen.height, screen.colorDepth, Intl.DateTimeFormat().resolvedOptions().timeZone, navigator.hardwareConcurrency || 'unknown', navigator.platform || 'unknown'];
...
async function secureFetch(method, params = {}, body = null) {
    ...
    if (uid) { headers['Authorization'] = `Bearer uid:${uid}`; }
    else if (sessionLogin && sessionPass) { headers['Authorization'] = `Bearer login:${btoa(unescape(encodeURIComponent(sessionLogin)))}:${sessionPass}`; }
...
async function syncToCloud() {
    ...
    if (isGuest && urlShareToken) { await fetch(`${WORKER_URL}?share=${urlShareToken}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ addresses }) }); return; }
...
function setAuthMode(mode) {
    authMode = mode;
    const loginBtn = $('authModeLogin'), regBtn = $('authModeRegister'), hint = $('authHint'), submitText = $('authBtnText');
    if (mode === 'login') {
        loginBtn?.classList.add('bg-white', 'dark:bg-[#2c2c2e]', 'text-slate-900', 'dark:text-white', 'shadow-sm');
        loginBtn?.classList.remove('text-slate-500');
        regBtn?.classList.remove('bg-white', 'dark:bg-[#2c2c2e]', 'text-slate-900', 'dark:text-white', 'shadow-sm');
        regBtn?.classList.add('text-slate-500');
        if (hint) hint.textContent = 'Введіть існуючий логін та пароль';
        if (submitText) submitText.textContent = 'Увійти';
        if ($('authLogin')) $('authLogin').placeholder = 'Ваш логін';
    } else {
        regBtn?.classList.add('bg-white', 'dark:bg-[#2c2c2e]', 'text-slate-900', 'dark:text-white', 'shadow-sm');
        regBtn?.classList.remove('text-slate-500');
        loginBtn?.classList.remove('bg-white', 'dark:bg-[#2c2c2e]', 'text-slate-900', 'dark:text-white', 'shadow-sm');
        loginBtn?.classList.add('text-slate-500');
        if (hint) hint.textContent = 'Придумайте логін та пароль — акаунт створюється автоматично';
        if (submitText) submitText.textContent = 'Створити акаунт';
        if ($('authLogin')) $('authLogin').placeholder = 'Придумайте логін';
    }
}
...
async function performLogin(rawLogin, rawPass, isAlreadyHashed, uid = null) {
    const errEl = $('authError'), spinner = $('authSpinner'), btnText = $('authBtnText');
    ...
    try {
        ...
        const data = await res.json();
        if (res.status === 404 && uid) { sessionLogin = prevLogin; sessionPass = prevPass; localStorage.removeItem('k_uid'); $('linkModal')?.classList.remove('hidden'); if (btnText) btnText.textContent = "Увійти"; if (spinner) spinner.classList.add('hidden'); return; }
        ...
        if (btnText) btnText.textContent = authMode === 'register' ? 'Створити акаунт' : 'Увійти';
        if (spinner) spinner.classList.add('hidden');
        if (errEl) { errEl.innerText = err.message === "WRONG_PASSWORD" ? "Неправильний пароль!" : "Помилка: " + err.message; errEl.classList.remove('hidden'); }
    }
}
...
async function linkAccount(lgn, pss) { const passHash = await getHash(pss); const res = await fetch(WORKER_URL, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: "link_google", login: lgn, pass: passHash, uid: googleUser.uid }) }); const data = await res.json(); if (data.success) { $('linkModal')?.classList.add('hidden'); showToast("Підв'язано!"); performLogin(null, null, false, googleUser.uid); } else showToast("Помилка", "❌"); }
$('btnLinkGoogle')?.addEventListener('click', async () => { if (!sessionLogin) return showToast("Спочатку увійдіть", "⚠️"); const provider = new firebase.auth.GoogleAuthProvider(); try { const result = await firebase.auth().signInWithPopup(provider); const uid = result.user.uid; const res = await fetch(WORKER_URL, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: "link_google", login: sessionLogin, uid }) }); if ((await res.json()).success) { showToast("Google підв'язано!"); localStorage.setItem('k_uid', uid); updateGoogleButton(); } } catch (e) { showToast("Скасовано", "⚠️"); } });
...
$('addAddressBtn')?.addEventListener('click', () => {
    ...
    const name = prompt("Назва об'єкту:"); if (name && name.trim()) { syncCurrentAddress(); const newId = 'addr_' + Date.now(); addresses.push({ id: newId, name: name.trim(), tariffs: { ...defaultTariffs }, prefs: { ...defaultPrefs }, records: [], customServices: [...defaultCustomServices] }); currentAddressId = newId; loadCurrentAddress(); syncToCloud(); closeAddressModal(); showToast("Додано"); checkNewAchievements(); }
});
function renderAddressModal() {
    ...
    list.querySelectorAll('[data-addr-id]').forEach(el => { el.addEventListener('click', (e) => { if (e.target.closest('.addr-edit') || e.target.closest('.addr-del')) return; syncCurrentAddress(); currentAddressId = el.dataset.addrId; loadCurrentAddress(); syncToCloud(); closeAddressModal(); }); });
    list.querySelectorAll('.addr-edit').forEach(btn => { btn.addEventListener('click', (e) => { e.stopPropagation(); const addr = addresses.find(a => a.id === btn.dataset.id); const name = prompt("Нова назва:", addr.name); if (name && name.trim()) { addr.name = name.trim(); renderAddressModal(); if (btn.dataset.id === currentAddressId) $('currentAddressDisplay').innerText = addr.name; syncToCloud(); } }); });
    list.querySelectorAll('.addr-del').forEach(btn => { btn.addEventListener('click', (e) => { e.stopPropagation(); if (confirm("Видалити?")) { addresses = addresses.filter(a => a.id !== btn.dataset.id); if (currentAddressId === btn.dataset.id) { currentAddressId = addresses[0].id; loadCurrentAddress(); } syncToCloud(); renderAddressModal(); } }); });
}
...
function switchTab(tabId, index) {
    const activeTab = document.querySelector('.tab-active'); const targetTab = $(tabId); if (!targetTab) return;
    ...
    setTimeout(() => { targetTab.classList.remove('tab-hidden'); targetTab.classList.add('tab-active'); requestAnimationFrame(() => { requestAnimationFrame(() => {
...
// =================== CANVAS CHART ENGINE ===================
class ChartEngine {
    constructor(canvasId, options = {}) { this.canvas = $(canvasId); if (!this.canvas) return; this.ctx = this.canvas.getContext('2d'); this.options = { padding: 40, barRadius: 8, animDuration: 600, unit: null, colors: { grid: 'rgba(0,0,0,0.05)', text: '#8e8e93' }, ...options }; this.data = []; this.animProgress = 0; this.tooltip = null; this.width = 0; this.height = 0; this.interactionBound = false; this.setupCanvas(); this.setupInteraction(); }
    setupCanvas() { if (!this.canvas) return; const dpr = window.devicePixelRatio || 1; const rect = this.canvas.getBoundingClientRect(); if (rect.width === 0 || rect.height === 0) return; this.canvas.width = rect.width * dpr; this.canvas.height = rect.height * dpr; this.ctx.setTransform(dpr, 0, 0, dpr, 0, 0); this.width = rect.width; this.height = rect.height; }
    setupInteraction() { if (!this.canvas || this.interactionBound) return; this.interactionBound = true; this.canvas.addEventListener('touchstart', (e) => this.handleTouch(e), { passive: true }); this.canvas.addEventListener('mousemove', (e) => this.handleHover(e)); this.canvas.addEventListener('mouseleave', () => { this.tooltip = null; this.render(); }); }
    ...
    findBar(x) { if (!this.data.length) return; const { padding } = this.options; const chartWidth = this.width - padding * 2; const barWidth = chartWidth / this.data.length; const index = Math.floor((x - padding) / barWidth); this.tooltip = (index >= 0 && index < this.data.length) ? { index, x: padding + index * barWidth + barWidth / 2 } : null; this.render(); }
    ...
    render() { if (!this.ctx || !this.width) return; const { ctx, width, height, data, options } = this; const { padding, barRadius, colors } = options; ctx.clearRect(0, 0, width, height); if (!data.length) { ctx.fillStyle = colors.text; ctx.font = '12px -apple-system'; ctx.textAlign = 'center'; ctx.fillText('Немає даних', width / 2, height / 2); return; } const chartWidth = width - padding * 2; const chartHeight = height - padding * 1.8; const max = Math.max(...data.map(d => d.value), 1); const barWidth = chartWidth / data.length; const barPad = barWidth * 0.25; ctx.strokeStyle = colors.grid; ctx.lineWidth = 0.5; for (let i = 0; i <= 3; i++) { const y = padding / 2 + (chartHeight / 3) * i; ctx.beginPath(); ctx.moveTo(padding, y); ctx.lineTo(width - padding, y); ctx.stroke(); } data.forEach((d, i) => { const barH = Math.max(2, (d.value / max) * chartHeight * this.animProgress); const x = padding + i * barWidth + barPad; const y = padding / 2 + chartHeight - barH; const w = barWidth - barPad * 2; const r = Math.min(barRadius, w / 2, barH / 2); ctx.shadowColor = d.color + '40'; ctx.shadowBlur = 8; ctx.shadowOffsetY = 4; ctx.beginPath(); ctx.moveTo(x, y + barH); ctx.lineTo(x, y + r); ctx.quadraticCurveTo(x, y, x + r, y); ctx.lineTo(x + w - r, y); ctx.quadraticCurveTo(x + w, y, x + w, y + r); ctx.lineTo(x + w, y + barH); ctx.closePath(); const grad = ctx.createLinearGradient(x, y, x, y + barH); grad.addColorStop(0, d.color); grad.addColorStop(1, d.color + '80'); ctx.fillStyle = grad; ctx.fill(); ctx.shadowColor = 'transparent'; ctx.shadowBlur = 0; ctx.shadowOffsetY = 0; if (this.tooltip && this.tooltip.index === i) { ctx.fillStyle = 'rgba(255,255,255,0.3)'; ctx.fill(); } ctx.fillStyle = colors.text; ctx.font = 'bold 9px -apple-system'; ctx.textAlign = 'center'; ctx.fillText(d.label, x + w / 2, height - 6); }); if (this.tooltip && this.tooltip.index < data.length) { const d = data[this.tooltip.index]; const tooltipText = this.options.unit ? `${d.value} ${this.options.unit}` : `${fmt.format(d.value)} ₴`; ctx.font = 'bold 11px -apple-system'; const tw = ctx.measureText(tooltipText).width + 16; const tx = Math.min(Math.max(this.tooltip.x - tw / 2, 4), width - tw - 4); ctx.fillStyle = 'rgba(0,0,0,0.85)'; ctx.beginPath(); if (ctx.roundRect) ctx.roundRect(tx, 4, tw, 22, 6); else ctx.rect(tx, 4, tw, 22); ctx.fill(); ctx.fillStyle = 'white'; ctx.textAlign = 'center'; ctx.fillText(tooltipText, tx + tw / 2, 19); } }
}
class DonutChart {
    ...
    render() { if (!this.ctx || !this.width) return; const { ctx, width, height, data } = this; ctx.clearRect(0, 0, width, height); if (!data.length) return; const cx = width / 2, cy = height / 2; const radius = Math.min(width, height) / 2 - 8; const innerRadius = radius * 0.6; const total = data.reduce((s, d) => s + d.value, 0); let startAngle = -Math.PI / 2; data.forEach(d => { const sliceAngle = (d.value / total) * Math.PI * 2 * this.animProgress; const endAngle = startAngle + sliceAngle; ctx.beginPath(); ctx.arc(cx, cy, radius, startAngle, endAngle); ctx.arc(cx, cy, innerRadius, endAngle, startAngle, true); ctx.closePath(); ctx.fillStyle = d.color; ctx.shadowColor = d.color + '30'; ctx.shadowBlur = 4; ctx.fill(); ctx.shadowColor = 'transparent'; ctx.shadowBlur = 0; startAngle = endAngle; }); const totalText = fmt.format(total); const fontSize = totalText.length > 9 ? 10 : totalText.length > 7 ? 12 : 14; ctx.fillStyle = document.documentElement.classList.contains('dark') ? '#fff' : '#1c1c1e'; ctx.font = `bold ${fontSize}px -apple-system`; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText(totalText, cx, cy - 4); ctx.fillStyle = '#8e8e93'; ctx.font = '9px -apple-system'; ctx.fillText('₴', cx, cy + 10); }
}
let dashChart, historyChart, serviceChart, donutChart;

// =================== DASHBOARD ===================
function renderDashboard() {
    const hour = new Date().getHours(); let greeting = 'Доброго дня!'; if (hour < 6) greeting = 'Доброї ночі!'; else if (hour < 12) greeting = 'Доброго ранку!'; else if (hour >= 18) greeting = 'Доброго вечора!';
    ...
    const unpaid = records.filter(r => !r.paid); const debtTotal = unpaid.reduce((s, r) => s + r.total, 0);
    if (unpaid.length > 0) { $('dashDebtCard')?.classList.remove('hidden'); animateNumber($('dashDebt'), debtTotal); if ($('dashDebtMonths')) $('dashDebtMonths').textContent = `${unpaid.length} міс.`; $('debtBadge')?.classList.remove('hidden'); if ($('debtBadge')) $('debtBadge').textContent = unpaid.length; }
    ...
    checkReminders(); checkBroadcast();
}
function renderBudgetProgress(curRec) { const budgetEl = $('budgetProgressCard'); if (!budgetEl) return; const budget = parseFloat(localStorage.getItem('k_budget')) || 0; if (!budget) { budgetEl.classList.add('hidden'); return; } budgetEl.classList.remove('hidden'); const spent = curRec ? curRec.total : 0; const percent = Math.min((spent / budget) * 100, 100); const remaining = Math.max(budget - spent, 0); const isOver = spent > budget; if ($('budgetSpent')) $('budgetSpent').textContent = fmt.format(spent); if ($('budgetLimit')) $('budgetLimit').textContent = fmt.format(budget); if ($('budgetRemaining')) { $('budgetRemaining').textContent = isOver ? `Перевищено на ${fmt.format(spent - budget)} ₴` : `Залишок: ${fmt.format(remaining)} ₴`; $('budgetRemaining').className = `text-[10px] font-bold ${isOver ? 'text-red-500' : 'text-green-600'}`; } const bar = $('budgetBar'); if (bar) { bar.style.width = `${percent}%`; bar.className = `h-full rounded-full transition-all duration-700 ${isOver ? 'bg-gradient-to-r from-red-400 to-red-600' : percent > 80 ? 'bg-gradient-to-r from-orange-400 to-orange-500' : 'bg-gradient-to-r from-brand to-blue-500'}`; } if ($('budgetPercent')) $('budgetPercent').textContent = `${Math.round(percent)}%`; }
function renderDonutChart(curRec) {
    if (!$('donutCanvas')) return; if (!donutChart) donutChart = new DonutChart('donutCanvas');
    ...
    if (curRec.drainageCost > 0) data.push({ value: curRec.drainageCost, color: '#14b8a6', label: 'Каналіз.' });
    ...
    const legend = $('donutLegend'); if (legend) legend.innerHTML = data.map(d => `<div class="flex items-center gap-1.5"><div class="w-2.5 h-2.5 rounded-full" style="background:${d.color}"></div><span class="text-[9px] font-bold text-slate-500">${d.label}</span></div>`).join('');
}
function renderDashCanvasChart() { if (!$('dashChartCanvas')) return; if (!dashChart) dashChart = new ChartEngine('dashChartCanvas', { padding: 24, barRadius: 6 }); const sorted = [...records].sort((a, b) => new Date(b.month) - new Date(a.month)).slice(0, 8).reverse(); const data = sorted.map(r => ({ value: r.total, label: new Date(r.month + '-01').toLocaleString('uk-UA', { month: 'short' }).slice(0, 3), color: r.paid ? '#007aff' : '#ff9500' })); dashChart.setData(data); }
function renderSmartInsight(curRec, curMonth) { const insightEl = $('dashInsight'); const textEl = $('dashInsightText'); if (!insightEl || !textEl) return; if (records.length < 2) { insightEl.classList.add('hidden'); return; } const sorted = [...records].sort((a, b) => new Date(b.month) - new Date(a.month)); const [sy, sm] = curMonth.split('-').map(Number); const prevDate = new Date(sy, sm - 2); const prevMonth = `${prevDate.getFullYear()}-${String(prevDate.getMonth() + 1).padStart(2, '0')}`; const prevRec = sorted.find(r => r.month === prevMonth); let insight = ''; if (curRec && prevRec && prevRec.total > 0) { const diff = Math.round(((curRec.total - prevRec.total) / prevRec.total) * 100); if (diff < -10) insight = `Зекономили ${Math.abs(diff)}% 🎉`; else if (diff > 15) insight = `Витрати +${diff}% vs минулий місяць`; else if (diff >= -10 && diff <= 5) insight = `Витрати стабільні 👍`; } if (!insight && records.length >= 3) { const avg = sorted.slice(0, 3).reduce((s, r) => s + r.total, 0) / 3; insight = `Середні за 3 міс: ${fmt.format(avg)} ₴`; } if (!insight) { const str = getStreak(records); if (str >= 3) insight = `Серія ${str} міс. 🔥`; } if (insight) { insightEl.classList.remove('hidden'); textEl.textContent = insight; } else { insightEl.classList.add('hidden'); } }
...
function animateNumber(el, target) { if (!el) return; const current = parseFloat(el.textContent.replace(/[^\d.,]/g, '').replace(',', '.')) || 0; if (Math.abs(current - target) < 0.01) { el.textContent = fmt.format(target) + ' ₴'; return; } const duration = 400; const start = performance.now(); const from = current; function tick(now) { const elapsed = now - start; const progress = Math.min(elapsed / duration, 1); const eased = 1 - Math.pow(1 - progress, 3); el.textContent = fmt.format(from + (target - from) * eased) + ' ₴'; if (progress < 1) requestAnimationFrame(tick); } requestAnimationFrame(tick); }

// =================== BROADCAST ===================
async function checkBroadcast() {
    try {
        const res = await fetch(`${WORKER_URL}?check=broadcast`, { cache: 'no-store' });
        const data = await res.json();
        if (!data.success || !data.broadcast) return;
        const { message, date } = data.broadcast;
        const lastSeen = localStorage.getItem('last_broadcast_seen');
        if (lastSeen === date) return;
        setTimeout(() => { showToast(message, '📢'); localStorage.setItem('last_broadcast_seen', date); }, 2000);
    } catch (e) {}
}
// =================== CALCULATION ===================
...
function calculatePreview() {
    ...
    if (prefs.showDrainage) { const coldVol = prefs.showWater ? Math.max(0, getV('wCur') - getV('wPrev')) : 0; const hotVol = prefs.showHotWater ? Math.max(0, getV('hwCur') - getV('hwPrev')) : 0; currentCalc.drainageCost = (coldVol + hotVol) * (tariffs.drainage || 0); } else currentCalc.drainageCost = 0;
    if (prefs.showElectro) {
        const dV = Math.max(0, getV('dCur') - getV('dPrev')); const nV = prefs.electroTwoZone ? Math.max(0, getV('nCur') - getV('nPrev')) : 0; const tEl = dV + nV;
        if (tEl === 0) currentCalc.electroCost = 0;
        else if (prefs.electroWinter && $('isWinterInput')?.checked) { if (tEl <= tariffs.winterLimit) currentCalc.electroCost = dV * tariffs.electroWinter + nV * tariffs.electroWinter * tariffs.nightCoef; else { const dp = dV / tEl, np = nV / tEl; currentCalc.electroCost = tariffs.winterLimit * dp * tariffs.electroWinter + tariffs.winterLimit * np * tariffs.electroWinter * tariffs.nightCoef + (tEl - tariffs.winterLimit) * dp * tariffs.electroBase + (tEl - tariffs.winterLimit) * np * tariffs.electroBase * tariffs.nightCoef; } }
    ...
    customServices.forEach(srv => { let val = parseFloat($(`custom_${srv.id}`)?.value); if (isNaN(val) && srv.defaultSum) val = parseFloat(srv.defaultSum); if (!isNaN(val) && val > 0) currentCalc.customCost += val; });
    currentCalc.total = currentCalc.waterCost + currentCalc.hotWaterCost + currentCalc.drainageCost + currentCalc.electroCost + currentCalc.gasCost + currentCalc.customCost;
    ...
    if ($('drainageCostDisplay')) $('drainageCostDisplay').innerText = fmt.format(currentCalc.drainageCost) + ' ₴';
    ...
    if ($('blockDrainage')) $('blockDrainage').style.display = (prefs.showDrainage && currentCalc.drainageCost > 0) ? 'block' : 'none';
...
let draftTimeout; function debouncedDraft() { clearTimeout(draftTimeout); draftTimeout = setTimeout(saveDraft, 1000); }
...
$('utilityForm')?.addEventListener('submit', (e) => {
    ...
    if (existingIdx >= 0) {
        ...
        merged.total = (merged.waterCost || 0) + (merged.hotWaterCost || 0) + (merged.drainageCost || 0) + (merged.electroCost || 0) + (merged.gasCost || 0) + (merged.customCost || 0);
        merged.note = newData.note || existing.note;
        records[existingIdx] = merged;
        showSaveSummary('Оновлено', merged);
    } else {
        records.push(newData);
        showSaveSummary('Збережено', newData);
    }

    clearDraft();
    $('submitFormBtn')?.classList.add('save-btn-success'); setTimeout(() => $('submitFormBtn')?.classList.remove('save-btn-success'), 600);
    syncToCloud();
    const [y, m] = $('monthInput').value.split('-'); const nD = new Date(y, m); $('monthInput').value = `${nD.getFullYear()}-${String(nD.getMonth() + 1).padStart(2, '0')}`;
...
function fillPreviousReadings() {
    try {
        ...
        if (prevRecord) { if (prefs.showWater && prevRecord.wCur != null && $('wPrev')) $('wPrev').value = prevRecord.wCur; if (prefs.showHotWater && prevRecord.hwCur != null && $('hwPrev')) $('hwPrev').value = prevRecord.hwCur; if (prefs.showElectro) { if (prevRecord.dCur != null && $('dPrev')) $('dPrev').value = prevRecord.dCur; if (prefs.electroTwoZone && prevRecord.nCur != null && $('nPrev')) $('nPrev').value = prevRecord.nCur; } if (prefs.showGas && prevRecord.gCur != null && $('gPrev')) $('gPrev').value = prevRecord.gCur; }
        ...
        const mo = new Date(selectedMonth + '-01').getMonth() + 1; if ($('isWinterInput')) $('isWinterInput').checked = mo >= 10 || mo <= 4;
...
function renderRecords() {
    ...
    renderServiceChart(); list.innerHTML = '';
    ...
    if (unpaidCount > 0 && currentFilter !== 'paid') { const batchBar = document.createElement('div'); batchBar.className = 'bg-gradient-to-r from-green-500 to-emerald-600 p-4 rounded-2xl flex justify-between items-center text-white mb-4'; batchBar.innerHTML = `<div><p class="text-xs font-bold opacity-80">${unpaidCount} неоплачених</p><p class="text-sm font-black">${fmt.format(sorted.filter(r => !r.paid).reduce((s, r) => s + r.total, 0))} ₴</p></div><button class="batch-pay-btn px-4 py-2 bg-white/20 rounded-xl text-xs font-bold active:scale-95 transition-transform border border-white/20">✓ Оплатити всі</button>`; list.appendChild(batchBar); batchBar.querySelector('.batch-pay-btn')?.addEventListener('click', () => { if (confirm(`Позначити ${unpaidCount} як оплачені?`)) { records.forEach(r => { if (!r.paid) r.paid = true; }); renderRecords(); renderDashboard(); syncToCloud(); checkNewAchievements(); showToast(`${unpaidCount} оплачено!`, '✅'); } }); }

    let lastYear = null;
    sorted.forEach(rec => { const yr = rec.month.split('-')[0]; if (yr !== lastYear) { lastYear = yr; const h = document.createElement('div'); h.className = "flex items-center gap-4 mt-6 mb-3"; h.innerHTML = `<h2 class="text-lg font-black text-slate-300 dark:text-slate-600">${yr}</h2><div class="h-[1px] flex-1 bg-slate-200 dark:bg-white/5"></div>`; list.appendChild(h); } list.appendChild(createRecordCard(rec)); });
}

function renderHistoryChart(sortedRecords, retryCount = 0) { if (!$('historyChartCanvas')) return; if (!historyChart) historyChart = new ChartEngine('historyChartCanvas', { padding: 30, barRadius: 5 }); if (!historyChart.width) historyChart.setupCanvas(); if (!historyChart.width) { if (retryCount < 5) setTimeout(() => renderHistoryChart(sortedRecords, retryCount + 1), 200); return; } const recent = sortedRecords.slice(-10); const data = recent.map(r => ({ value: r.total, label: new Date(r.month + '-01').toLocaleString('uk-UA', { month: 'short' }).slice(0, 3), color: r.paid ? '#007aff' : '#ff9500' })); historyChart.setData(data); }
function renderServiceChart(retryCount = 0) { if (!$('serviceChartCanvas') || records.length === 0) { if ($('serviceChartSummary')) $('serviceChartSummary').innerHTML = ''; return; } const type = $('serviceChartSelect')?.value || 'water'; const unit = (type === 'electro') ? 'кВт' : 'м³'; if (!serviceChart) serviceChart = new ChartEngine('serviceChartCanvas', { padding: 24, barRadius: 4, unit }); else serviceChart.options.unit = unit; if (!serviceChart.width) serviceChart.setupCanvas(); if (!serviceChart.width) { if (retryCount < 5) setTimeout(() => renderServiceChart(retryCount + 1), 200); return; } const sorted = [...records].sort((a, b) => new Date(a.month) - new Date(b.month)).slice(-8); const getValue = (rec) => { switch (type) { case 'water': return Math.max(0, (rec.wCur || 0) - (rec.wPrev || 0)); case 'hotWater': return Math.max(0, (rec.hwCur || 0) - (rec.hwPrev || 0)); case 'electro': return Math.max(0, (rec.dCur || 0) - (rec.dPrev || 0)) + Math.max(0, (rec.nCur || 0) - (rec.nPrev || 0)); case 'gas': return Math.max(0, (rec.gCur || 0) - (rec.gPrev || 0)); default: return 0; } }; const getColor = () => { switch (type) { case 'water': return '#3b82f6'; case 'hotWater': return '#ef4444'; case 'electro': return '#eab308'; case 'gas': return '#f97316'; default: return '#6b7280'; } }; const color = getColor(); const data = sorted.map(rec => ({ value: getValue(rec), label: new Date(rec.month + '-01').toLocaleString('uk-UA', { month: 'short' }).slice(0, 3), color })); serviceChart.setData(data); const values = data.map(d => d.value); const avg = values.length ? values.reduce((a, b) => a + b, 0) / values.length : 0; const last = values[values.length - 1] || 0; const prevLast = values.length > 1 ? values[values.length - 2] : last; const trendPct = prevLast > 0 ? Math.round(((last - prevLast) / prevLast) * 100) : 0; const summary = $('serviceChartSummary'); if (summary) summary.innerHTML = `<span>Сер.: <span style="color:${color}" class="font-black">${Math.round(avg)} ${unit}/міс</span></span><span>Ост.: <span class="${trendPct < 0 ? 'text-green-600' : trendPct > 0 ? 'text-red-500' : 'text-slate-500'} font-black">${last} ${unit} (${trendPct > 0 ? '+' : ''}${trendPct}%)</span></span>`; }
...
function createRecordCard(rec) {
    ...
    card.innerHTML = `<div class="flex justify-between items-center relative z-10" data-toggle-details="${recId}"><div><h4 class="font-bold text-lg capitalize text-slate-900 dark:text-white mb-1">${escapeHtml(dStr)}</h4><div class="flex items-center flex-wrap gap-1"><span class="text-[9px] font-black uppercase tracking-widest px-2.5 py-1 rounded-lg ${rec.paid ? 'bg-brand-light text-brand' : 'bg-orange-100 text-orange-700 dark:bg-orange-500/20 dark:text-orange-400'}">${rec.paid ? 'Оплачено' : 'Борг'}</span>${partialBadge}</div></div><div class="flex items-center gap-2"><span class="font-black text-xl text-slate-900 dark:text-white">${fmt.format(rec.total)} ₴</span><i class="chevron-icon fa-solid fa-chevron-down text-slate-400 text-xs transition-transform duration-300"></i></div></div><div class="details-panel hidden"><div class="border-t border-slate-100 dark:border-white/5 pt-4 mt-4 space-y-2 text-sm">${rec.waterCost > 0 ? `<div class="flex justify-between"><span>💧 Вода</span><span class="font-bold">${fmt.format(rec.waterCost)} ₴</span></div>` : ''}${rec.hotWaterCost > 0 ? `<div class="flex justify-between"><span>🌡️ Гар. вода</span><span class="font-bold">${fmt.format(rec.hotWaterCost)} ₴</span></div>` : ''}${rec.drainageCost > 0 ? `<div class="flex justify-between"><span>🚿 Каналізація</span><span class="font-bold">${fmt.format(rec.drainageCost)} ₴</span></div>` : ''}${rec.electroCost > 0 ? `<div class="flex justify-between"><span>⚡ Світло</span><span class="font-bold">${fmt.format(rec.electroCost)} ₴</span></div>` : ''}${rec.gasCost > 0 ? `<div class="flex justify-between"><span>🔥 Газ</span><span class="font-bold">${fmt.format(rec.gasCost)} ₴</span></div>` : ''}${rec.customCost > 0 ? `<div class="flex justify-between"><span>📦 Фіксовані</span><span class="font-bold">${fmt.format(rec.customCost)} ₴</span></div>${rec.customData ? Object.values(rec.customData).filter(s => s.val > 0).map(s => `<div class="flex justify-between text-xs text-slate-500 pl-4"><span>${escapeHtml(s.name)}</span><span>${fmt.format(s.val)} ₴</span></div>`).join('') : ''}` : ''}${rec.note ? `<div class="mt-2 p-2 bg-slate-50 dark:bg-black/50 rounded-lg text-xs text-slate-500 italic">${escapeHtml(rec.note)}</div>` : ''}</div><div class="flex gap-2 mt-4 pt-3 border-t border-slate-100 dark:border-white/5"><button type="button" class="rec-pay flex-1 py-3 rounded-xl font-bold text-xs active:scale-[0.96] transition-all ${rec.paid ? 'bg-slate-100 dark:bg-[#2c2c2e] text-slate-500' : 'bg-brand text-white shadow-md'}">${rec.paid ? '↩ Скасувати' : '✓ Оплачено'}</button><button type="button" class="rec-share px-4 py-3 bg-blue-50 dark:bg-blue-500/10 rounded-xl text-blue-500 active:scale-90"><i class="fa-solid fa-share-nodes"></i></button><button type="button" class="rec-edit px-4 py-3 bg-slate-50 dark:bg-white/5 rounded-xl text-slate-400 active:scale-90"><i class="fa-solid fa-pen"></i></button><button type="button" class="rec-del px-4 py-3 bg-red-50 dark:bg-red-500/10 rounded-xl text-red-400 active:scale-90"><i class="fa-solid fa-trash"></i></button></div></div>`;
    ...
    card.addEventListener('click', (e) => {
        const target = e.target.closest('[data-toggle-details]'); if (target) { const panel = card.querySelector('.details-panel'); const chevron = card.querySelector('.chevron-icon'); if (panel) { panel.classList.toggle('hidden'); if (chevron) chevron.style.transform = panel.classList.contains('hidden') ? '' : 'rotate(180deg)'; } return; }
        if (e.target.closest('.rec-pay')) { e.stopPropagation(); togglePaidById(recId); return; }
        if (e.target.closest('.rec-share')) { e.stopPropagation(); shareRecordById(recId); return; }
        if (e.target.closest('.rec-edit')) { e.stopPropagation(); editRecordById(recId); return; }
        if (e.target.closest('.rec-del')) { e.stopPropagation(); if (confirm('Видалити?')) deleteRecordById(recId); return; }
    });
    return card;
}

async function shareRecordById(id) { const rec = records.find(r => r.id === id); if (!rec) return; const d = new Date(rec.month + '-01').toLocaleString('uk-UA', { month: 'long', year: 'numeric' }); let t = `🧾 Комуналка за ${d}\n──────────\n`; if (rec.waterCost > 0) t += `💧 Вода: ${fmt.format(rec.waterCost)} ₴\n`; if (rec.hotWaterCost > 0) t += `🌡️ Гар.: ${fmt.format(rec.hotWaterCost)} ₴\n`; if (rec.drainageCost > 0) t += `🚿 Каналіз.: ${fmt.format(rec.drainageCost)} ₴\n`; if (rec.electroCost > 0) t += `⚡ Світло: ${fmt.format(rec.electroCost)} ₴\n`; if (rec.gasCost > 0) t += `🔥 Газ: ${fmt.format(rec.gasCost)} ₴\n`; if (rec.customCost > 0) t += `📦 Інше: ${fmt.format(rec.customCost)} ₴\n`; t += `──────────\n💰 Всього: ${fmt.format(rec.total)} ₴\n${rec.paid ? '✅ Оплачено' : '⏳ Очікує'}`; if (navigator.share) { try { await navigator.share({ text: t }); return; } catch (e) {} } try { await navigator.clipboard.writeText(t); showToast("Скопійовано!", "📋"); } catch (e) { prompt(":", t); } }
...
let searchDebounce; $('searchRecords')?.addEventListener('input', () => { clearTimeout(searchDebounce); searchDebounce = setTimeout(renderRecords, 200); });
...
function applyPreferences() {
    ...
    if ($('prefDrainage')) $('prefDrainage').checked = prefs.showDrainage;
...
['prefWater', 'prefHotWater', 'prefDrainage', 'prefElectro', 'prefGas', 'prefElectroTwoZone', 'prefElectroWinter'].forEach(id => { $(id)?.addEventListener('change', () => { prefs.showWater = $('prefWater')?.checked ?? prefs.showWater; prefs.showHotWater = $('prefHotWater')?.checked ?? prefs.showHotWater; prefs.showDrainage = $('prefDrainage')?.checked ?? prefs.showDrainage; prefs.showElectro = $('prefElectro')?.checked ?? prefs.showElectro; prefs.showGas = $('prefGas')?.checked ?? prefs.showGas; prefs.electroTwoZone = $('prefElectroTwoZone')?.checked ?? prefs.electroTwoZone; prefs.electroWinter = $('prefElectroWinter')?.checked ?? prefs.electroWinter; applyPreferences(); renderCalcCustomServices(); calculatePreview(); updateSmartBadges(); }); });
...
// City selector
$('citySelect')?.addEventListener('change', function() { if (this.value) applyCityTariffs(this.value); });

$('saveSettingsBtn')?.addEventListener('click', () => {
    tariffs = { water: parseFloat($('tWater')?.value) || defaultTariffs.water, hotWater: parseFloat($('tHotWater')?.value) || defaultTariffs.hotWater, drainage: parseFloat($('tDrainage')?.value) || defaultTariffs.drainage, electroBase: parseFloat($('tElectroBase')?.value) || defaultTariffs.electroBase, electroWinter: parseFloat($('tElectroWinter')?.value) || defaultTariffs.electroWinter, winterLimit: 2000, nightCoef: 0.5, gas: parseFloat($('tGas')?.value) || defaultTariffs.gas };
    prefs = { showWater: $('prefWater')?.checked, showHotWater: $('prefHotWater')?.checked, showDrainage: $('prefDrainage')?.checked, showElectro: $('prefElectro')?.checked, showGas: $('prefGas')?.checked, electroTwoZone: $('prefElectroTwoZone')?.checked, electroWinter: $('prefElectroWinter')?.checked, remindersEnabled: $('prefReminders')?.checked, remWaterStart: parseInt($('remWaterStart')?.value) || 1, remWaterEnd: parseInt($('remWaterEnd')?.value) || 5, remElectroStart: parseInt($('remElectroStart')?.value) || 28, remElectroEnd: parseInt($('remElectroEnd')?.value) || 3 };
...
function generateYearReport() { const year = new Date().getFullYear(); const yr = records.filter(r => r.month.startsWith(String(year))); if (!yr.length) { showToast('Немає даних за рік', '⚠️'); return; } if ($('yearReportYear')) $('yearReportYear').textContent = year; const total = yr.reduce((s, r) => s + r.total, 0); const avg = total / yr.length; const maxR = yr.reduce((a, b) => a.total > b.total ? a : b); const minR = yr.reduce((a, b) => a.total < b.total ? a : b); const paid = yr.filter(r => r.paid).length; const wT = yr.reduce((s, r) => s + (r.waterCost || 0), 0); const hwT = yr.reduce((s, r) => s + (r.hotWaterCost || 0), 0); const eT = yr.reduce((s, r) => s + (r.electroCost || 0), 0); const gT = yr.reduce((s, r) => s + (r.gasCost || 0), 0); const cT = yr.reduce((s, r) => s + (r.customCost || 0), 0); const maxM = new Date(maxR.month + '-01').toLocaleString('uk-UA', { month: 'long' }); const minM = new Date(minR.month + '-01').toLocaleString('uk-UA', { month: 'long' }); const streak = getStreak(records); let html = `<div class="text-center mb-2"><p class="text-3xl font-black text-slate-900 dark:text-white">${fmt.format(total)} ₴</p><p class="text-[10px] text-slate-400 font-bold uppercase tracking-wider mt-1">Загальні витрати</p></div><div class="grid grid-cols-2 gap-3"><div class="bg-slate-50 dark:bg-black/40 p-3 rounded-xl text-center"><p class="text-[9px] font-bold text-slate-400 uppercase">Середній</p><p class="text-lg font-black text-slate-900 dark:text-white">${fmt.format(avg)} ₴</p></div><div class="bg-slate-50 dark:bg-black/40 p-3 rounded-xl text-center"><p class="text-[9px] font-bold text-slate-400 uppercase">Місяців</p><p class="text-lg font-black text-slate-900 dark:text-white">${yr.length}</p></div></div><div class="bg-green-50 dark:bg-green-500/10 p-4 rounded-2xl"><div class="flex justify-between"><span class="text-sm font-bold text-green-700 dark:text-green-400">📉 Мін</span><span class="font-black text-green-700 dark:text-green-400">${fmt.format(minR.total)} ₴</span></div><p class="text-[10px] text-green-600/70 mt-0.5">${minM}</p></div><div class="bg-red-50 dark:bg-red-500/10 p-4 rounded-2xl"><div class="flex justify-between"><span class="text-sm font-bold text-red-700 dark:text-red-400">📈 Макс</span><span class="font-black text-red-700 dark:text-red-400">${fmt.format(maxR.total)} ₴</span></div><p class="text-[10px] text-red-600/70 mt-0.5">${maxM}</p></div><div class="bg-slate-50 dark:bg-black/40 p-4 rounded-2xl"><p class="text-[9px] font-bold text-slate-400 uppercase mb-3">Розподіл</p><div class="space-y-2">${wT > 0 ? `<div class="flex justify-between text-xs"><span>💧 Вода</span><span class="font-black">${fmt.format(wT)} ₴ (${Math.round(wT / total * 100)}%)</span></div>` : ''}${hwT > 0 ? `<div class="flex justify-between text-xs"><span>🌡️ Гар.</span><span class="font-black">${fmt.format(hwT)} ₴ (${Math.round(hwT / total * 100)}%)</span></div>` : ''}${eT > 0 ? `<div class="flex justify-between text-xs"><span>⚡ Світло</span><span class="font-black">${fmt.format(eT)} ₴ (${Math.round(eT / total * 100)}%)</span></div>` : ''}${gT > 0 ? `<div class="flex justify-between text-xs"><span>🔥 Газ</span><span class="font-black">${fmt.format(gT)} ₴ (${Math.round(gT / total * 100)}%)</span></div>` : ''}${cT > 0 ? `<div class="flex justify-between text-xs"><span>📦 Інше</span><span class="font-black">${fmt.format(cT)} ₴ (${Math.round(cT / total * 100)}%)</span></div>` : ''}</div></div><div class="grid grid-cols-2 gap-3"><div class="bg-brand-light p-3 rounded-xl text-center border border-brand-border"><p class="text-[9px] font-bold text-brand uppercase">Оплачено</p><p class="text-lg font-black text-brand">${paid}/${yr.length}</p></div><div class="bg-orange-50 dark:bg-orange-500/10 p-3 rounded-xl text-center"><p class="text-[9px] font-bold text-orange-500 uppercase">Серія</p><p class="text-lg font-black text-orange-500">${streak} 🔥</p></div></div>`; if ($('yearReportContent')) $('yearReportContent').innerHTML = html; $('yearReportModal')?.classList.remove('hidden'); haptic('success'); }
async function shareYearReport() { const year = new Date().getFullYear(); const yr = records.filter(r => r.month.startsWith(String(year))); if (!yr.length) return; const total = yr.reduce((s, r) => s + r.total, 0); const avg = total / yr.length; const streak = getStreak(records); let t = `📊 Річний звіт ${year}\n═══════\n💰 Всього: ${fmt.format(total)} ₴\n📈 Середній: ${fmt.format(avg)} ₴/міс\n📅 Записів: ${yr.length}\n🔥 Серія: ${streak} міс.`; if (navigator.share) { try { await navigator.share({ text: t }); return; } catch (e) {} } try { await navigator.clipboard.writeText(t); showToast("Скопійовано!", "📋"); } catch (e) { prompt(":", t); } }
...
// =================== PUSH ===================
async function initPush() { if (!('Notification' in window)) return; const btn = $('enablePushBtn'); const st = $('pushStatus'); if (Notification.permission === 'granted') { if (btn) btn.classList.add('hidden'); if (st) { st.classList.remove('hidden'); st.textContent = '✓ Push увімкнено'; st.className = 'text-[10px] text-green-500 text-center font-bold'; } } else if (Notification.permission !== 'denied') { if (btn) btn.classList.remove('hidden'); } }
...
// =================== SHARE APP ===================
$('shareAppBtn')?.addEventListener('click', async () => { const text = '🏠 Комуналка — облік комунальних.\nhttps://komynalka.vercel.app'; if (navigator.share) { try { await navigator.share({ text, url: 'https://komynalka.vercel.app' }); return; } catch (e) {} } try { await navigator.clipboard.writeText(text); showToast('Скопійовано!', '📋'); } catch (e) { prompt(':', text); } });
...
function initAppUI() {
    ...
    updateGoogleButton(); applyPreferences(); renderCalcCustomServices(); fillPreviousReadings();
    switchTab('tabDashboard', 0); calculatePreview(); updateSmartBadges(); renderDashboard();
    // Hide calc hint if dismissed
    if (localStorage.getItem('calcHintDismissed') && $('calcHint')) $('calcHint').classList.add('hidden');
...
// =================== AUTO-LOGIN ===================
if (urlShareToken) {
    isGuest = true; $('authScreen')?.classList.add('hidden'); $('appScreen')?.classList.remove('hidden'); $('appScreen')?.classList.add('flex');
    ...
    fetch(`${WORKER_URL}?share=${urlShareToken}`, { cache: "no-store" }).then(r => r.json()).then(data => { if (data.success) { addresses = data.data.addresses; currentAddressId = data.data.currentAddressId; loadCurrentAddress(); } else alert("Посилання недійсне."); });
}
...
// Preconnect
const preconnect = document.createElement('link'); preconnect.rel = 'preconnect'; preconnect.href = WORKER_URL; document.head.appendChild(preconnect);
