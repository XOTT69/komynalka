...
function attachBottomNavToViewport() {
  const bottomNav = $('bottomNav');
  if (bottomNav && bottomNav.parentElement !== document.body) document.body.appendChild(bottomNav);
}
attachBottomNavToViewport();
...
let dashChart, historyChart, serviceChart, analyticsChart;
...
function renderDashboard() {
  ...
  renderDashCanvasChart(); renderBudgetProgress(curRec); renderSmartInsight(curRec,curMonth); renderMonthMiniWidget(curRec,curMonth); renderAchievements(); renderTips();
...
// =================== RESIZE ===================
let resizeTimeout;
window.addEventListener('resize',()=>{clearTimeout(resizeTimeout);resizeTimeout=setTimeout(()=>{[dashChart,historyChart,serviceChart,analyticsChart].forEach(chart=>{if(chart&&chart.canvas){chart.setupCanvas();if(chart.width)chart.render();}});},250);});

// =================== LAZY CHARTS ===================
const chartObserver=new IntersectionObserver(entries=>{entries.forEach(entry=>{if(!entry.isIntersecting)return;const id=entry.target.id;if(id==='dashChartCanvas'&&dashChart){dashChart.setupCanvas();dashChart.render();}if(id==='historyChartCanvas'&&historyChart){historyChart.setupCanvas();historyChart.render();}if(id==='serviceChartCanvas'&&serviceChart){serviceChart.setupCanvas();serviceChart.render();}if(id==='analyticsChartCanvas'&&analyticsChart){analyticsChart.setupCanvas();analyticsChart.render();}});},{threshold:0.1});
['dashChartCanvas','historyChartCanvas','serviceChartCanvas','analyticsChartCanvas'].forEach(id=>{const el=$(id);if(el)chartObserver.observe(el);});
