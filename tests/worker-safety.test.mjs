...
test('private admin entry is visible only to the authenticated owner',async()=>{
  const {env}=environment({xott69:legacyAccount(hash),anna:legacyAccount(hash)});
  env.ADMIN_OWNER_LOGIN='xott69';
  const access=authorization=>worker.fetch(new Request('https://test.workers.dev/?admin-access=1',{headers:authorization?{Authorization:authorization}:{}}),env);
  assert.equal((await access()).status,401);
  assert.equal((await access(auth('xott69').replace(hash,'0'.repeat(64)))).status,403);
  assert.deepEqual(await (await access(auth('anna'))).json(),{success:true,allowed:false});
  assert.deepEqual(await (await access(auth('xott69'))).json(),{success:true,allowed:true});
